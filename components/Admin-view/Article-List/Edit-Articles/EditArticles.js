...
import { COMMENT_API_KEY, BLOG_POST_USER_API_KEY, CURENT_USER_LOGIN_API, REACTION_NEWSFEED_API_KEY, BOOKMARK_NEWSFEED_API_KEY, NEWSFEED_COMMENT_POST_KEY, SEARCH_MULTIPLE, HASHTAGS_API, ADMIN_BLOG_API } from "../../../../pages/config";
import { Popover, Transition } from "@headlessui/react";
import PostComments from "../../../blog/comments/PostComments";
...
function EditArticles() {
  ...
  const getBlogs = async () => {
      ...
      try {
        if (result.status == 200) {
          ...
          setBlogContent(result.data.data.description);
          setblog_type(result.data.data.blog_type);
          console.log(result.data.data.blog_type);
        }
      } catch (error) {}
      setLoading(false);
      return result;
  ...
  function Updateblog() {
    ...
    setSpinner(true);
    fetch(ADMIN_BLOG_API+ "/" + myArray[1], {
      method: "PUT",
      headers: {
        Accept: "application/json",
        Authorization: `${authKey}`,
        ...
        <div className="image">
          ...
          ( //  Show Blog-Pic
            <div className="relative flex">
              ...
              {currentUser && list && list.data && currentUser.role=="admin"?(
