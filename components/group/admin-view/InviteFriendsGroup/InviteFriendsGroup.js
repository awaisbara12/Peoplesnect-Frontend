...
import { GROUP_API, GROUP_MEMBERS_API, InviteFriends } from "../../../../pages/config";
...
const InviteFriendsGroup = (props) => {
  ...
  //   const res = await axios(InviteFriends+"/get_friends_and_followers?group_id="+props.group.id, {
  //     method: "GET",
  //     headers: {
  ...
  const GetMember = () => {
    fetch(GROUP_MEMBERS_API + "?group_id=" + props.group.id, {
      method: "GET",
      headers: {
        Accept: "application/json",
        Authorization: `${authKey}`,
      ...
      .then((result) => {
        // setList(result.data);
        const mergedList = result.data.map(({ group_member }) => group_member);
